"""PydanticAI agent for DSM interview plans."""
...
from app.agents.schemas import DsmInterviewPlanOutput
...
SYSTEM_PROMPT = """
Monte plano de entrevista DSM estruturado, orientado por chunks recuperados.
Use somente as ferramentas controladas permitidas. Não invente critérios.
Inclua perguntas que mapeiem critérios, duração, prejuízo, exclusões e especificadores apenas quando houver evidência.
Sempre retorne used_chunks e limitações se a recuperação for fraca.
"""


def create_interview_agent(model: str) -> PydanticAgent[DsmAgentDeps, DsmInterviewPlanOutput] | None:
    try:
        from pydantic_ai import Agent, RunContext
    except ModuleNotFoundError:
        return None

    agent = Agent(model, deps_type=DsmAgentDeps, output_type=DsmInterviewPlanOutput, instructions=SYSTEM_PROMPT)

    @agent.tool
    async def search_dsm_chunks(ctx: RunContext[DsmAgentDeps], query: str | None = None, top_k: int | None = None) -> list[dict]:
        """Search DSM chunks for interview evidence."""
